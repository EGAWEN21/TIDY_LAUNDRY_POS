  ...
  <div class="tw-h-[calc(100vh-4rem)] dark:tw-bg-slate-900/50 tw-bg-white/50 tw-backdrop-blur-xl tw-shadow-[-4px_0_24px_rgba(0,0,0,0.05)] tw-border-l tw-border-white/20 dark:tw-border-white/5 tw-p-4 lg:tw-p-6" :class="shown && detached ? 'tw-absolute tw-inset-0 tw-w-full tw-z-50' : 'tw-hidden lg:tw-block lg:tw-w-7/12 tw-w-full tw-shrink-0'">
      ...
      <div class="tw-flex tw-items-center tw-gap-2 tw-mt-1 tw-p-2 tw-w-full tw-h-14">
          ...
          <button :disabled="isSyncing" class="tw-px-2 tw-justify-center tw-font-semibold tw-py-2 tw-h-full bg-primary-600 tw-rounded-md tw-text-white tw-flex tw-items-center tw-gap-1.5 tw-w-full tw-border-0 tw-shadow-md disabled:tw-opacity-50" @click.prevent="$emit('syncAndPrint')">
              ...
              <span>{{ isSyncing ? 'Syncing...' : 'Save & Print' }}</span>
          </button>
          <button class="tw-px-2 tw-py-2 bg-danger-600 tw-rounded-md tw-text-white tw-h-full tw-flex tw-items-center tw-gap-1.5 tw-border-0 tw-shadow-md" @click.prevent="$emit('clearAll')">
